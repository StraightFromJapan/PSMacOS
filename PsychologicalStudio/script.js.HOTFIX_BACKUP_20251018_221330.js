                    ...
                    if (button) {
                        ...
                        if (sampleData.buffer) {
                            base64ToAudioBuffer(sampleData.buffer)
                                ...
                                .catch(error => {
                                    console.warn(`⚠️ Could not load buffer for sample ${sampleNumber}:`, error);
                                    button.classList.add("no-sample");
                                });
                        }
                ...
                if (typeof window !== 'undefined' && window.electronAPI && typeof window.electronAPI.selectFolder === 'function') {
                    ...
                    if (folderPath) {
                        ...
                        if (typeof window.electronAPI.loadFolderAudioFiles === 'function') {
                            console.log(`📁 Requesting audio file list...`);
                            const folderResult = await window.electronAPI.loadFolderAudioFiles(folderPath);
                            
                            console.log(`📁 Got folderResult:`, folderResult);
                            ...
                            if (audioFiles && audioFiles.length > 0) {
                                ...
                                if (result.success && result.loadedCount > 0) {
                                    ...
                                    showNotification(`✅ Loaded ${result.loadedCount} audio files from folder`);
                                    console.log(`✅ Loaded ${result.loadedCount}/${result.totalCount} files successfully`);
                                } else {
